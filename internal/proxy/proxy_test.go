...
func TestResolvePublicIPs(t *testing.T) {
	resolver := &mockResolver{addrs: map[string][]net.IPAddr{
		...
		"multi.example.com":        {ip("93.184.216.34"), ip("1.1.1.1")},
		...
		"cgnat.example.com":        {ip("100.64.0.1")},
		"docv6.example.com":        {ip("2001:db8::1")},
	...
	tests := []struct {
		...
		{"CGNAT", "cgnat.example.com", false, 0, "DNS rebinding"},
		{"IPv6 documentation", "docv6.example.com", false, 0, "DNS rebinding"},
